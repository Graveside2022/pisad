...
class CreateFrequencyProfileRequest(BaseModel):
    ...
    center_frequency_hz: int = Field(..., ge=1_000_000, le=6_000_000_000)  # 1 MHz to 6 GHz
...
async def get_frequency_profiles() -> FrequencyProfilesResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"ASV configuration manager not available: {e}"
        )
    ...
    try:
        ...
        for profile in all_profiles.values():
            profile_responses.append(FrequencyProfileResponse(
                name=profile.name,
        ...
        try:
            ...
            logger.info("ASV coordinator integration pending for active profile detection")
        except Exception as e:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve frequency profiles: {e!s}"
        )


@router.post("/switch-frequency", response_model=FrequencySwitchResponse)
async def switch_frequency_profile(request: FrequencySwitchRequest) -> FrequencySwitchResponse:
    ...
    try:
        ...
        success = True
        logger.info(f"Frequency switching simulated for {profile.center_frequency_hz/1e6:.3f} MHz")
        ...
        if switch_time_ms > 50:
            logger.warning(f"Frequency switching took {switch_time_ms:.1f}ms, exceeds 50ms requirement")
...
async def get_current_frequency() -> CurrentFrequencyResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get current frequency: {e!s}"
        )


@router.post("/frequency-profiles", response_model=dict[str, Any])
async def create_frequency_profile(request: CreateFrequencyProfileRequest) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create frequency profile: {e!s}"
        )
...
async def delete_frequency_profile(profile_name: str) -> dict[str, Any]:
    ...
    if profile_name in protected_profiles:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot delete built-in profile: {profile_name}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to delete frequency profile: {e!s}"
        )
