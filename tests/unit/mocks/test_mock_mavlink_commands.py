...
class TestMockMAVLinkCommands:
    ...
    def mock_mavlink_service(self):
        ...
        mock = MagicMock()
        mock.connected = True
        mock.get_telemetry = MagicMock(return_value={
            "armed": False,
            "flight_mode": "GUIDED",
            "altitude": 0.0,
    ...
    def mock_safety_system(self):
        ...
        mock = MagicMock()
        mock.check_all_interlocks = MagicMock(return_value={
            "all_safe": True,
            "gps_ready": True,
            "battery_ok": True,
            "geofence_ok": True,
    ...
    async def test_velocity_command(self, command_pipeline):
        ...
        command_id = await command_pipeline.submit_command(
            ...
            parameters={"vx": 5.0, "vy": 3.0, "vz": -1.0},
            source="test"
        )
        assert command_id is not None
    ...
    async def test_homing_commands(self, command_pipeline):
        ...
        command_id = await command_pipeline.submit_command(
            ...
            parameters={"search_pattern": "spiral"},
            source="test"
        )
        assert command_id is not None
    ...
    def test_command_priority_ordering(self):
        ...
        critical = Command(
            type=CommandType.RETURN_TO_LAUNCH,
            priority=CommandPriority.CRITICAL
        )
        normal = Command(
