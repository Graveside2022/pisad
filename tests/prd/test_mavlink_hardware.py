...
from backend.services.mavlink_service import MAVLinkService, ConnectionState
...
class TestMAVLinkRequirements:
    ...
    def mock_connection(self):
        """Create mock MAVLink connection for testing."""
        # Use pymavlink's test utilities
    ...
    def test_fr9_telemetry_streaming(self, mavlink_service):
        ...
        expected_time = 2.0  # 10 packets at 5Hz = 2 seconds
    ...
    def test_nfr1_packet_loss(self, mavlink_service, mock_connection):
        ...
        # Setup mock to return heartbeat messages
        heartbeat_msg = Mock()
        heartbeat_msg.get_type.return_value = 'HEARTBEAT'
        mock_connection.recv_match = Mock(return_value=heartbeat_msg)
        ...
        for i in range(packets_sent):
            ...
            if mavlink_service.connection:
                ...
                # Simulate receiving response (in real scenario, this would be async)
                msg = mock_connection.recv_match.return_value
                if msg:
                    packets_received += 1
            else:
    ...
    def test_fr11_gcs_override(self, mavlink_service):
        ...
        assert result.get('status') in ['rejected', 'queued'], f"Unexpected status: {result.get('status')}"
    ...
    async def test_telemetry_with_detection(self, mavlink_service):
        ...
        assert hasattr(mavlink_service, 'send_telemetry') or hasattr(mavlink_service, '_telemetry_queue'), \
