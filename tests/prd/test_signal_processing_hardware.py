...
from backend.services.signal_processor import SignalProcessor, EWMAFilter
...
class TestSignalProcessingHardware:
    ...
    def generate_beacon_signal(self, 
                              frequency: float = 406.025e6,
                              duration: float = 0.5,
                              sample_rate: float = 2.048e6) -> np.ndarray:
        ...
        bit_pattern = np.random.choice([-1, 1], size=int(duration * bit_rate))
        
        # Upsample bit pattern
        modulation = np.repeat(bit_pattern, int(sample_rate / bit_rate))[:num_samples]
    ...
    def test_fr6_ewma_filtering(self, signal_processor):
        ...
        assert filtered_variance < noise_variance * 0.7, \
    ...
    def test_nfr2_processing_latency(self, signal_processor):
        ...
        print(f"Processing latency: avg={avg_latency:.2f}ms, max={max_latency:.2f}ms, p95={p95_latency:.2f}ms")
    ...
    def test_fr7_debounced_transitions(self, signal_processor):
        ...
        # Brief spike should not trigger detection
        test_sequence = [
            (-70, False),  # Below threshold
            (-50, False),  # Above threshold (1st)
            (-48, False),  # Above threshold (2nd)
            (-70, False),  # Below threshold - reset
            (-50, False),  # Above threshold (1st again)
            (-48, False),  # Above threshold (2nd)
    ...
    def test_adaptive_threshold_calculation(self, signal_processor):
        ...
        noise_history = [
            -95, -94, -95, -96, -95,  # Stable noise
            -90, -89, -88, -87, -86,  # Increasing noise
            -95, -94, -95, -96, -95,  # Back to stable
        ]
