...
class TestConfigDataclasses:
    ...
    def test_logging_config_defaults(self):
        ...
        assert config.LOG_ENABLE_CONSOLE
        assert config.LOG_ENABLE_FILE
        assert config.LOG_ENABLE_JOURNAL
...
class TestConfigLoader:
    ...
    def test_load_default_config(self):
        ...
        assert config.app.APP_PORT == 8000  # From default.yaml
        # Note: custom.yaml profile overrides SDR frequency
        assert config.sdr.SDR_FREQUENCY == 433000000  # From custom.yaml profile
        assert config.sdr.SDR_SAMPLE_RATE == 2000000  # From custom.yaml profile
        assert config.logging.LOG_LEVEL == "INFO"
        assert config.safety.SAFETY_VELOCITY_MAX_MPS == 5.0  # From custom.yaml profile
    ...
    def test_load_from_yaml_file(self):
        ...
        try:
            ...
            # Note: SDR_FREQUENCY gets overridden by custom profile
    ...
    def test_environment_override(self):
        ...
        try:
            ...
            assert config.development.DEV_DEBUG_MODE
        finally:
    ...
    def test_boolean_conversion(self):
        ...
        for env_value, expected in test_cases:
            ...
            try:
                loader = ConfigLoader()
                config = loader.load()
                assert expected == config.development.DEV_MOCK_SDR, f"Failed for {env_value}"
            finally:
                del os.environ["PISAD_DEV_MOCK_SDR"]

    def test_list_conversion(self):
        """Test list value conversion from environment."""
        os.environ["PISAD_API_CORS_ORIGINS"] = (
        ...
        try:
            ...
            assert expected == config.api.API_CORS_ORIGINS
        finally:
