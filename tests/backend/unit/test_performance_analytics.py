...
class TestDetectionMetricsCalculation:
    ...
    def test_calculate_detection_metrics_with_authentic_data(self):
        ...
        for i, (minutes, confidence) in enumerate(zip(detection_times, confidences)):
        ...
        metrics = service.calculate_detection_metrics(telemetry_data, detection_events, search_area_km2)
        
        # Verify detection metrics
        assert metrics.total_detections == 3
        assert abs(metrics.detections_per_hour - 1.5) < 0.1  # ~1.5 detections per hour (allow for precision)
    ...
    def test_detection_metrics_confidence_calculation(self):
        ...
        metrics_mixed = service.calculate_detection_metrics(telemetry_data, detection_events_mixed, 10.0)
    ...
    def test_detection_coverage_calculation(self):
        ...
        small_area = 1.0  # 1 km²
        metrics_small = service.calculate_detection_metrics(telemetry_data, detection_events, small_area)
        
        large_area = 100.0  # 100 km²
        metrics_large = service.calculate_detection_metrics(telemetry_data, detection_events, large_area)
...
class TestFalsePositiveNegativeAnalysis:
    ...
    def test_analyze_false_positives_mixed_scenario(self):
        ...
        detections = [
            ...
            {"location": {"lat": 37.7740, "lon": -122.4184}},  # False positive (not near any beacon)
        ]
...
class TestEnvironmentalCorrelation:
    ...
    def test_rf_noise_correlation_calculation(self):
        ...
        detection_events = [
            {"rssi": -60, "snr": 20, "timestamp": start_time.isoformat()},  # Strong signal, high SNR
            {"rssi": -70, "snr": 15, "timestamp": start_time.isoformat()},  # Medium signal, medium SNR  
            {"rssi": -80, "snr": 10, "timestamp": start_time.isoformat()},  # Weak signal, low SNR
            {"rssi": -90, "snr": 5, "timestamp": start_time.isoformat()},   # Very weak signal, very low SNR
        ]
    ...
    def test_weather_impact_scoring(self):
        ...
        bad_weather = {
            ...
            "precipitation_mm": 5.0    # Heavy rain
        }
...
class TestPerformanceReportGeneration:
    ...
    def test_generate_performance_report_comprehensive_data(self):
        ...
        ground_truth_beacons = [{
            "latitude": beacon_location[0],
            "longitude": beacon_location[1]
        }]
    ...
    def test_overall_score_calculation(self):
        ...
        score = service._calculate_overall_score(
            detection_metrics, approach_metrics, search_metrics,
            false_positive_analysis, baseline_comparison
        )
    ...
    def test_recommendations_generation(self):
        ...
        detection_metrics = DetectionMetrics()
        detection_metrics.mean_detection_confidence = 60.0  # Low confidence - should trigger recommendation
        detection_metrics.first_detection_time = 400.0     # Slow detection - should trigger recommendation
        
        approach_metrics = ApproachMetrics()  
        approach_metrics.approach_efficiency = 60.0        # Poor efficiency - should trigger recommendation
        approach_metrics.rssi_improvement_db = 5.0         # Poor RSSI improvement - should trigger recommendation
        
        search_metrics = SearchMetrics()
        search_metrics.coverage_percentage = 70.0          # Poor coverage - should trigger recommendation
        search_metrics.search_pattern_efficiency = 60.0   # Poor pattern - should trigger recommendation
        ...
        environmental_correlation.rf_noise_correlation = 0.4  # Poor correlation - should trigger recommendation
        environmental_correlation.weather_impact_score = 60.0  # Poor weather - should trigger recommendation
        
        recommendations = service._generate_recommendations(
            detection_metrics, approach_metrics, search_metrics,
            false_positive_analysis, environmental_correlation
        )
...
class TestHelperMethods:
    ...
    def test_calculate_distance_accuracy(self):
        ...
        assert 10 < small_distance < 15  # Should be approximately 11 meters for 0.0001 degree difference
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_empty_and_none_inputs(self):
        ...
        detection_metrics = service.calculate_detection_metrics(empty_telemetry, empty_detections, 10.0)
        ...
        env_correlation = service.correlate_environmental_factors(empty_telemetry, empty_detections, None)
