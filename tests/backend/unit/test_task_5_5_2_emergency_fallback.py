...
class TestSeamlessDroneOnlyFallback:
    ...
    async def test_seamless_transition_without_flight_interruption(self, coordinator):
        ...
        ), f"Transition took {transition_time_ms:.1f}ms, should be <100ms"
    ...
    @pytest.mark.asyncio
    async def test_operator_notification_system_for_fallback_activation(
        self, coordinator, bridge_service
    ):
    ...
    async def test_integration_with_communication_loss_detection(self, coordinator, bridge_service):
        ...
        bridge_service._last_communication = (
