...
class TestEnhancedHomingAlgorithmIntegration:
    ...
    def confidence_homing(self, mock_asv_processor, homing_algorithm):
        ...
        return ASVConfidenceBasedHoming(
            asv_processor=mock_asv_processor, homing_algorithm=homing_algorithm
        )
    ...
    def test_dynamic_threshold_adjustment_overrides_static_threshold(
        ...
        assert (
            decision.confidence_assessment == "MODERATE"
        )  # Updated based on combined score calculation
    ...
    def test_asv_signal_degradation_detection_integration(self, confidence_homing):
    ...
    def test_configuration_integration_with_existing_parameters(self, confidence_homing):
        ...
        assert (
            decision.dynamic_threshold <= custom_config.high_quality_threshold + 0.1
        )  # Some adjustment expected
