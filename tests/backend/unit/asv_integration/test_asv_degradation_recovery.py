...
class TestASVDegradationDetection:
    ...
    def test_degradation_detection_confidence_threshold_breached(self):
        ...
        for i, (confidence, strength) in enumerate(
            zip(confidence_history, signal_strength_history)
        ):
        ...
        assert (
            degradation_event.severity == DegradationSeverity.SIGNIFICANT
    ...
    def test_degradation_detection_no_false_positives_stable_signal(self):
        ...
        detector = ASVDegradationDetector(
            confidence_threshold=0.3, trend_window_size=5, degradation_rate_threshold=0.15
        )
    ...
    def test_degradation_detection_with_interference_penalty(self):
        ...
        asv_metrics_with_interference.confidence = (
            0.45  # Above threshold, but interference should push below
        )
        ...
        assert (
            degradation_event.interference_penalty_applied
        ), "Interference penalty should be applied"
        assert (
            degradation_event.effective_confidence < 0.4
        ), "Effective confidence should be penalized"
...
class TestASVSafetyAuthorityIntegration:
    ...
    async def test_recovery_strategy_respects_safety_authority_levels(self):
        ...
        safety_manager = Mock()
        safety_manager.get_current_authority_level = Mock(
            return_value=SafetyAuthorityLevel.GEOFENCE
        )
    ...
    async def test_recovery_blocked_by_emergency_stop_authority(self):
        ...
        safety_manager = Mock()
        safety_manager.get_current_authority_level = Mock(
            return_value=SafetyAuthorityLevel.EMERGENCY_STOP
        )
