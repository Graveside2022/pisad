...
from src.backend.services.asv_integration.exceptions import ASVHardwareError
...
class TestASVFrequencyProfileAPI:
    ...
    def test_get_frequency_profiles_returns_all_profiles(self, mock_config_manager):
        ...
        with patch(
            "src.backend.services.asv_integration.asv_configuration_manager.ASVConfigurationManager"
        ) as mock_asv_config:
            ...
            emergency_profile = next(
                p for p in data["profiles"] if p["name"] == "emergency_beacon_406"
            )
    ...
    def test_switch_frequency_profile_endpoint_exists(self):
        ...
        response = client.post(
            "/api/asv/switch-frequency", json={"profile_name": "emergency_beacon_406"}
        )
    ...
    async def test_switch_frequency_profile_success(self, mock_config_manager, mock_coordinator):
        ...
        with patch(
            ...
            response = client.post(
                "/api/asv/switch-frequency", json={"profile_name": "aviation_emergency"}
            )
    ...
    def test_switch_frequency_profile_invalid_profile(self):
        ...
        response = client.post(
            "/api/asv/switch-frequency", json={"profile_name": "nonexistent_profile"}
        )
    ...
    def test_switch_frequency_profile_hardware_error(self, mock_config_manager, mock_coordinator):
        ...
        with (
            ...
            mock_coordinator.switch_frequency.side_effect = ASVHardwareError(
                "HackRF communication failed"
            )
    ...
    def test_frequency_validation_out_of_range(self):
        ...
        response = client.post(
            "/api/asv/switch-frequency", json={"profile_name": "invalid_low_frequency"}
        )
    ...
    def test_create_custom_frequency_profile(self, mock_config_manager):
        ...
        with patch(
            "src.backend.api.routes.asv_integration.asv_config_manager", mock_config_manager
        ):
            custom_profile_data = {
                "name": "test_custom",
    ...
    def test_performance_requirement_frequency_switching(
        self, mock_config_manager, mock_coordinator
    ):
