...
class TestASVHackRFCoordinator:
    ...
    async def test_coordinator_initialization(
        self, mock_config_manager, mock_safety_authority, hackrf_config
    ):
    ...
    async def test_frequency_selection_algorithm(self, mock_config_manager):
        ...
        # Mock analyzers
        coordinator._active_analyzers = {"emergency_gp": MagicMock(), "vor_nav": MagicMock()}
    ...
    async def test_safety_validation_integration(self, mock_safety_authority):
        ...
        assert (
            call_args.kwargs["operation_context"]["operation_type"] == "asv_analyzer_coordination"
        )
    ...
    async def test_analyzer_management_api(self, mock_config_manager):
        ...
        with patch(
            "src.backend.services.asv_integration.asv_hackrf_coordinator.ASVAnalyzerFactory"
        ) as MockFactory:
            # Setup mock factory
            ...
            config = ASVAnalyzerConfig(
                frequency_hz=406_000_000, ref_power_dbm=-120.0, analyzer_type="GP"
            )
    ...
    def test_active_analyzers_query(self, mock_config_manager):
        ...
        coordinator._active_analyzers = {"emergency_beacon": mock_gp, "vor_nav": mock_vor}
