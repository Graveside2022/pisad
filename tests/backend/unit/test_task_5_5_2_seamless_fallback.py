...
@pytest.fixture
def coordinator_with_deps(mock_tcp_bridge, mock_signal_processor, mock_safety_manager) -> DualSDRCoordinator:
...
class TestTask5_5_2_2_G1_ComprehensiveStateManagement:
    ...
    async def test_fallback_mode_comprehensive_state_initialization(self, coordinator_with_deps):
        ...
        assert hasattr(coordinator, '_last_fallback_time') or hasattr(coordinator, '_fallback_start_time'), \
    ...
    async def test_fallback_state_validation_and_integrity_checks(self, coordinator_with_deps):
        ...
        assert health_status["active_source"] == "drone", "Health status must show drone as active source"
        ...
        assert health_status["coordination_active"] == coordinator.is_running, \
...
class TestTask5_5_2_2_G2_AutomaticSourceSwitching:
    ...
    async def test_automatic_source_switching_to_drone(self, coordinator_with_deps):
        ...
        assert coordinator.fallback_active is True, \
            "Fallback must be active after source switching"
...
class TestTask5_5_2_2_H_SeamlessTransition:
    ...
    async def test_zero_interruption_rssi_source_switching(self, coordinator_with_deps):
        ...
        assert transition_rssi == -50.0, "Drone RSSI should be immediately available after transition"
        ...
        transition_time = (time.time() - coordinator._fallback_start_time) * 1000
        assert transition_time < 50, f"Transition must complete in <50ms, took {transition_time:.1f}ms"
    ...
    async def test_smooth_coordination_handover_preserves_continuity(self, coordinator_with_deps):
        ...
        health_status = await coordinator.get_health_status()
        assert health_status["coordination_active"] is True, "Health status must show active coordination"
        ...
        handover_time = (time.time() - coordination_start_time) * 1000
        assert handover_time < 100, f"Complete handover must be under 100ms, took {handover_time:.1f}ms"
    
    @pytest.mark.asyncio
    async def test_transition_timing_optimization_meets_latency_requirements(self, coordinator_with_deps):
        """
        ...
        for scenario in scenarios:
            ...
            # Verify immediate compliance with <100ms requirement
            assert transition_time_ms < 100, f"Transition for {scenario} took {transition_time_ms:.1f}ms (>100ms limit)"
        ...
        assert avg_time < 50, f"Average transition time {avg_time:.1f}ms should be well under 100ms"
        assert max_time < 100, f"Maximum transition time {max_time:.1f}ms must meet requirement"
        
        # Performance should be predictable (low variance)
        time_variance = max(transition_times) - min(transition_times)
        assert time_variance < 30, f"Transition timing variance {time_variance:.1f}ms should be low for predictability"
    ...
    async def test_transition_state_monitoring_and_validation(self, coordinator_with_deps):
        ...
        assert "coordination_active" in initial_health, "Health status must include coordination state"
        ...
        assert coordinator._fallback_state_validated is True, "State must be validated during transition"
    ...
    async def test_seamless_recovery_coordination_when_ground_restored(self, coordinator_with_deps):
        ...
        assert coordinator.active_source == "ground", "Should recover to ground source with better signal"
        
        # Verify recovery timing meets requirements
        recovery_time_ms = (recovery_end_time - recovery_start_time) * 1000
        assert recovery_time_ms < 100, f"Recovery transition took {recovery_time_ms:.1f}ms (>100ms limit)"
        
        # Verify coordination continues after recovery
        health_status = await coordinator.get_health_status()
        assert "coordination_active" in health_status, "Coordination must remain active after recovery"
...
class TestTask5_5_2_2_I_FallbackStatusMonitoring:
    ...
    @pytest.mark.asyncio
    async def test_enhanced_health_status_with_detailed_fallback_metrics(self, coordinator_with_deps):
        """
        ...
        assert "fallback_duration_ms" not in initial_health or initial_health["fallback_duration_ms"] == 0
        assert "fallback_trigger_count" not in initial_health or initial_health["fallback_trigger_count"] == 0
        ...
        assert enhanced_health["fallback_start_time"] > fallback_start - 1, "Start time should be accurate"
    ...
    @pytest.mark.asyncio
    async def test_fallback_duration_tracking_and_performance_analytics(self, coordinator_with_deps):
        """
        ...
        # Duration should be reasonably accurate (within 10ms tolerance)
        assert abs(reported_duration - measured_duration) < 10, \
    ...
    @pytest.mark.asyncio
    async def test_fallback_status_logging_with_safety_event_correlation(self, coordinator_with_deps):
        """
        ...
        for reason in safety_reasons:
            ...
            assert hasattr(coordinator, '_fallback_scenarios'), "Scenarios must be tracked for logging"
        ...
        assert len(coordinator._fallback_scenarios) == len(safety_reasons), "All scenarios must be logged"
    ...
    async def test_fallback_status_reporting_to_safety_manager(self, coordinator_with_deps):
        ...
        assert hasattr(safety_manager, 'handle_communication_loss'), \
        ...
        assert fallback_status["fallback_reason"] == "safety_reporting_test", "Reason must be available"
    ...
    async def test_fallback_monitoring_dashboard_integration(self, coordinator_with_deps):
        ...
        required_fields = [
            "fallback_active", "active_source", "coordination_active",
            "fallback_duration_ms", "fallback_trigger_count", "fallback_reason"
        ]
        ...
        assert isinstance(dashboard_metrics["fallback_duration_ms"], (int, float)), \
            "Duration must be numeric"
        assert isinstance(dashboard_metrics["fallback_trigger_count"], int), \
            "Trigger count must be integer"
        assert isinstance(dashboard_metrics["coordination_latency_ms"], (int, float)), \
            "Latency must be numeric"
        
        # Verify boolean fields
        assert isinstance(dashboard_metrics["fallback_active"], bool), \
            "Fallback active must be boolean"
        assert isinstance(dashboard_metrics["coordination_active"], bool), \
...
class TestTask5_5_2_2_J_GracefulDegradation:
    ...
    async def test_feature_availability_matrix_for_drone_only_mode(self, coordinator_with_deps):
        ...
        for feature in expected_normal_features:
            assert feature in normal_features["available"], \
                f"Feature {feature} should be available in normal mode"
        
        assert normal_features["mode"] == "dual_source", "Mode should be dual_source"
        assert len(normal_features["available"]) >= 8, "Normal mode should have comprehensive features"
        ...
        for feature in expected_fallback_features:
            assert feature in fallback_features["available"], \
        ...
        for feature in unavailable_in_fallback:
            assert feature not in fallback_features["available"], \
                f"Feature {feature} should NOT be available in fallback mode"
            assert feature in fallback_features.get("unavailable", []), \
                f"Feature {feature} should be listed as unavailable"
        
        assert fallback_features["mode"] == "drone_only", "Mode should be drone_only"
        assert fallback_features["degradation_level"] == "partial", "Should indicate partial degradation"
    
    @pytest.mark.asyncio
    async def test_intelligent_feature_disabling_with_user_notification(self, coordinator_with_deps):
        """
        ...
        assert "dual_source_coordination" not in features["available"], \
            "Dual source coordination should be disabled"
        assert "ground_sdr_integration" not in features["available"], \
            "Ground SDR integration should be disabled"
        
        # Check that essential features remain available
        assert "drone_source_only" in features["available"], \
            "Drone source should remain available"
        assert "safety_fallback_operation" in features["available"], \
        ...
        assert degradation_notification is not None, "Feature degradation notification should be sent"
        assert "disabled_features" in degradation_notification["details"], \
            "Notification should include disabled features list"
        assert "available_features" in degradation_notification["details"], \
    ...
    async def test_coordination_overhead_reduction_in_fallback_mode(self, coordinator_with_deps):
        ...
        assert fallback_duration <= normal_duration * 1.2, \
        ...
        assert avg_fallback_time < 0.1, \
            f"Average fallback coordination time should be under 100ms: {avg_fallback_time:.4f}s"
    ...
    async def test_graceful_degradation_status_reporting(self, coordinator_with_deps):
        ...
        assert degradation_status["degradation_level"] in ["partial", "significant"], \
        ...
        assert degradation_status["degradation_reason"] == "status_reporting_test", \
            "Reason should match trigger reason"
        assert degradation_status["disabled_features_count"] > 0, \
            "Some features should be disabled"
        assert degradation_status["available_features_count"] > 0, \
            "Some features should still be available"
        
        # Verify performance impact classification
        assert degradation_status["performance_impact_level"] in ["low", "medium", "high"], \
    ...
    async def test_fallback_mode_optimization_for_single_sdr(self, coordinator_with_deps):
        ...
        assert avg_optimization_time < 0.05, \
            f"Average optimization should be under 50ms: {avg_optimization_time:.4f}s"
        assert max_optimization_time < 0.1, \
            f"Maximum optimization should be under 100ms: {max_optimization_time:.4f}s"
...
class TestTask5_5_2_2_K_OperatorNotificationSystem:
    ...
    @pytest.mark.asyncio
    async def test_real_time_fallback_activation_notifications_via_websocket(self, coordinator_with_deps):
        """
        ...
        assert hasattr(tcp_bridge, "auto_notify_communication_issue"), \
        ...
        assert hasattr(coordinator, '_fallback_reason'), "Fallback reason must be tracked for notifications"
        assert coordinator._fallback_reason == "websocket_notification_test", \
            "Notification reason must match trigger"
    
    @pytest.mark.asyncio
    async def test_fallback_status_integration_with_safety_manager_notifications(self, coordinator_with_deps):
        """
        ...
        assert safety_event["event_type"] in ["coordination_fallback", "feature_degradation"], \
        ...
        if "details" in safety_event:
            ...
            assert "degradation_level" in details, "Safety notification must include degradation level"
    ...
    async def test_fallback_alert_severity_classification_and_routing(self, coordinator_with_deps):
        ...
        for reason, expected_severity in severity_scenarios:
            ...
            assert "performance_impact_level" in degradation_status, \
            ...
            if expected_severity in ["high", "critical"]:
                assert impact_level in ["medium", "high"], \
    ...
    async def test_fallback_notification_rate_limiting_and_aggregation(self, coordinator_with_deps):
        ...
        assert len(notification_times) == len(rapid_triggers), \
        ...
        assert total_time < 1.0, f"Notification processing should be under 1 second: {total_time:.3f}s"
        
        # Verify notification aggregation in trigger count
        assert coordinator._fallback_trigger_count == len(rapid_triggers), \
    ...
    async def test_operator_acknowledgment_system_for_fallback_events(self, coordinator_with_deps):
        ...
        assert health_status["fallback_reason"] == "acknowledgment_test", \
            "Acknowledgment system must track specific fallback reason"
        assert health_status["fallback_start_time"] > 0, \
        ...
        if hasattr(coordinator, '_fallback_scenarios'):
            ...
            assert "timestamp" in latest_scenario, "Scenario must include timestamp for acknowledgment"
...
class TestTask5_5_2_2_L_AutomaticRecovery:
    ...
    @pytest.mark.asyncio
    async def test_ground_communication_restoration_detection_with_validation(self, coordinator_with_deps):
        """
    ...
    @pytest.mark.asyncio
    async def test_automatic_coordination_restoration_with_integrity_checks(self, coordinator_with_deps):
        """
        ...
        assert not hasattr(coordinator, '_recovery_failure_flag'), \
            "Recovery should not have integrity check failures"
    
    @pytest.mark.asyncio
    async def test_recovery_transition_smoothing_to_prevent_oscillation(self, coordinator_with_deps):
        """
        ...
        assert coordinator.fallback_active is False, "Should remain in recovered state (no oscillation)"
        ...
        assert avg_recovery_time < 0.05, f"Recovery smoothing should not add excessive delay: {avg_recovery_time:.4f}s"
    ...
    @pytest.mark.asyncio
    async def test_recovery_completion_notification_and_status_reporting(self, coordinator_with_deps):
        """
        ...
        original_handle = getattr(coordinator._safety_manager, 'handle_communication_restored', None)
        ...
        if hasattr(coordinator._safety_manager, 'handle_communication_restored'):
            assert len(recovery_notifications) > 0, "Recovery completion should generate notifications"
            
            notification = recovery_notifications[0]
            assert notification["type"] == "recovery_completed", "Notification type should indicate completion"
            assert notification["active_source"] == "ground", "Notification should show recovered source"
            assert notification["fallback_active"] is False, "Notification should show fallback disabled"
        ...
        degradation_status = await coordinator.get_degradation_status()
        assert degradation_status["degradation_active"] is False, "Degradation should be cleared after recovery"
        assert degradation_status["degradation_level"] == "none", "Degradation level should be none after recovery"
