...
from backend.utils.logging import (
    ...
    clear_correlation_id, LogContext, CorrelationIdFilter, StructuredFormatter,
    log_info, log_error, log_warning, log_debug, log_critical,
    PerformanceLogger, log_with_context
)
...
class TestLoggingSetup:
    ...
    def test_setup_journal_logging(self):
        ...
        with patch.dict('sys.modules', {'systemd': mock_systemd, 'systemd.journal': mock_systemd.journal}):
            setup_logging(
                enable_console=False,
                enable_file=False,
                enable_journal=True
    ...
    def test_log_level_setting(self):
        ...
        for level_str in test_levels:
            setup_logging(log_level=level_str, enable_console=True, enable_file=False, enable_journal=False)
            root_logger = logging.getLogger()
