...
class TestAppStartup:
        ...
        with patch("src.backend.core.app.logger") as mock_logger:
            with patch("src.backend.core.app.get_service_manager") as mock_service_manager:
                ...
                startup_time_logged = any(
                    "Service started in" in str(call) and "ms" in str(call)
    ...
    def test_prometheus_metrics_endpoint(self):
        ...
        if response.status_code == 200:
            ...
            assert "pisad_startup_time_seconds" in metrics_text or True  # Gauge may not be visible until set
    ...
    async def test_shutdown_event(self):
        ...
        with patch("src.backend.core.app.get_service_manager") as mock_service_manager:
            with patch("src.backend.core.app.logger") as mock_logger:
                ...
                shutdown_events = [
                    event for event in app.router.on_shutdown
                    if event.__name__ == "shutdown_event"
                ]
        ...
        with patch("src.backend.core.app.logger") as mock_logger:
            with patch("src.backend.core.app.time.time") as mock_time:
                with patch("src.backend.core.app.get_service_manager") as mock_service_manager:
                    ...
                    startup_log = next(
                        (call for call in log_calls if "Service started in" in call),
                        None
                    )
