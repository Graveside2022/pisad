...
class TestASVEnhancedGradientCalculation:
    ...
    def test_asv_enhanced_gradient_calculation_replaces_numpy(self, homing_algorithm, asv_integration):
        """
        ...
        assert homing_algorithm._asv_integration is not None, "ASV integration should be initialized"
    ...
    def test_asv_bearing_precision_improvement(self, homing_algorithm, asv_integration):
        ...
        assert bearing_error <= 2.0, f"ASV bearing error {bearing_error:.1f}° should be ≤2° (target precision)"
        
        # Verify confidence is high for ASV calculation
        assert gradient.confidence > 90.0, f"ASV confidence {gradient.confidence:.1f}% should be >90%"
    ...
    def test_gradient_vector_compatibility_preserved(self, homing_algorithm):
        ...
        assert 0.0 <= gradient.direction <= 360.0, f"Direction {gradient.direction}° should be 0-360°"
    ...
    def test_asv_integration_initialization(self, homing_algorithm):
        ...
        assert hasattr(asv_integration, 'calculate_enhanced_gradient'), "Should have enhanced calculation method"
    ...
    def test_performance_requirement_maintained(self, homing_algorithm):
        ...
        assert calculation_time_ms < 100.0, f"ASV calculation took {calculation_time_ms:.2f}ms, should be <100ms"