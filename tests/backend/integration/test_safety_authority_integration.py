...
class TestSafetyAuthorityIntegration:
    ...
    def sdrpp_bridge(self):
        ...
        bridge.get_communication_health = AsyncMock(return_value={
            "connected": True,
            "latency_ms": 30.0,
            "quality": 0.95,
        })
    ...
    @pytest.mark.asyncio
    async def test_safety_decision_validation_with_coordination(self, dual_coordinator, sdrpp_bridge):
        ...
        approved, reason, approving_authority = await safety_authority.validate_safety_decision(decision)
    ...
    async def test_authority_hierarchy_coordination_integration(self):
        ...
        approved, reason, approving_authority = await safety_authority.validate_safety_decision(geofence_decision)
    ...
    async def test_response_time_requirements_with_coordination(self):
        ...
        emergency_decision = SafetyDecision(
            ...
            details={
                ...
                "coordination_components": ["DualSDRCoordinator", "SDRPriorityManager", "SDRPPBridge"],
            },
        )
        
        approved, reason, approving_authority = await safety_authority.validate_safety_decision(emergency_decision)
        ...
        start_time = asyncio.get_event_loop().time()
        approved, reason, approving_authority = await safety_authority.validate_safety_decision(flight_mode_decision)
        end_time = asyncio.get_event_loop().time()
