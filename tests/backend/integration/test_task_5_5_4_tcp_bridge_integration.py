...
class TestSafetyTCPBridgeIntegration:
    ...
    async def test_fallback_timing_validation(
        ...
        assert (
            fallback_time < 10.0
        ), f"Fallback detection took {fallback_time:.2f}s, exceeds 10s limit"
    ...
    async def test_concurrent_tcp_safety_operations(
        ...
        tcp_task = asyncio.create_task(self._run_tcp_operations(sdrpp_bridge_service))
        safety_task = asyncio.create_task(self._run_safety_monitoring(safety_manager))
