...
class TestSDRPPSITLIntegration:
    ...
    @pytest.fixture
    async def sitl_sdrpp_service(
        self, unused_tcp_port, sitl_signal_processor, sitl_mavlink_service
    ):
    ...
    async def test_sitl_environment_setup_and_teardown(self, sitl_sdrpp_service):
        ...
        handshake = {"type": "heartbeat", "timestamp": time.time(), "client_id": "sitl_test_client"}
    ...
    async def test_system_performance_under_sitl_scenarios(self, sitl_sdrpp_service):
        ...
        tasks.append(
            asyncio.create_task(self._continuous_rssi_monitoring(service.port, scenario_duration))
        )

        # Task 2: Periodic frequency changes
        tasks.append(
            asyncio.create_task(self._periodic_frequency_control(service.port, scenario_duration))
        )

        # Task 3: Connection resilience testing
        tasks.append(
            asyncio.create_task(self._connection_resilience_test(service.port, scenario_duration))
        )
    ...
    async def _continuous_rssi_monitoring(self, port, duration):
        ...
        try:
            while time.time() - start_time < duration:
                ...
                request = {"type": "rssi_update", "timestamp": request_time}
                writer.write(json.dumps(request).encode() + b"\n")
                await writer.drain()
        ...
        try:
            while time.time() - start_time < duration:
                ...
                if response_data:
                    response = json.loads(response_data.decode().strip())
                    if (
                        response["type"] == "freq_control_response"
    ...
    async def _connection_resilience_test(self, port, duration):
        ...
        return {"reconnections": reconnections, "final_status": final_status}
