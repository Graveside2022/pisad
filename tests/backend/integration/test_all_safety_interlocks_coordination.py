...
from src.backend.utils.safety import SafetyInterlockSystem, SafetyTrigger
...
class TestAllSafetyInterlocksWithCoordination:
    ...
    def dual_coordinator(self):
        ...
        coordinator.trigger_emergency_override = AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_mode_check_interlock_with_coordination(
    ...
    @pytest.mark.asyncio
    async def test_signal_loss_interlock_with_coordination(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_battery_interlock_with_coordination(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_geofence_interlock_with_coordination(
        self, safety_system, dual_coordinator
    ):
        ...
        safety_results = await safety_system.check_all_safety()
        assert safety_results["geofence"] is False, "Geofence check should fail with altitude violation"
    ...
    async def test_coordination_health_interlock_with_coordination(
        ...
        dual_coordinator.get_health_status = AsyncMock(return_value={
    ...
    async def test_dual_source_signal_interlock_with_coordination(
        ...
        safety_results = await safety_system.check_all_safety()
        assert safety_results["dual_source_signal"] is True, "Dual source should pass with both good"
        ...
        safety_results = await safety_system.check_all_safety()
        assert safety_results["dual_source_signal"] is False, "Dual source should fail with both bad"

    @pytest.mark.asyncio
    async def test_all_interlocks_together_with_coordination(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_emergency_stop_affects_all_interlocks(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_interlock_timing_with_coordination(
        self, safety_system, dual_coordinator
    ):
        ...
        expected_checks = [
            "mode", "operator", "signal", "battery", "geofence",
            "coordination_health", "dual_source_signal"
        ]
        for check_name in expected_checks:
            assert check_name in safety_results, f"Missing check: {check_name}"

    @pytest.mark.asyncio
    async def test_interlock_independence_with_coordination(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_coordination_disabled_interlock_behavior(
        self, safety_system, dual_coordinator
    ):
