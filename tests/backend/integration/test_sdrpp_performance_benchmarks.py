...
class TestSDRPPPerformanceBenchmarks:
    ...
    @pytest.mark.asyncio
    async def test_json_message_processing_performance(
        self, benchmark, sdrpp_service_with_real_processor
    ):
        ...
        def json_processing_benchmark():
            ...
            test_message = {
                "type": "rssi_update",
                "timestamp": "2025-08-18T23:59:59Z",
                "data": {"rssi": -55.2, "snr": 12.5, "confidence": 0.95},
                "sequence": 12345,
            }
    ...
    @pytest.mark.asyncio
    async def test_concurrent_client_performance_impact(
        self, benchmark, sdrpp_service_with_real_processor
    ):
    ...
    @pytest.mark.asyncio
    async def test_sustained_operation_performance_degradation(
        self, benchmark, sdrpp_service_with_real_processor
    ):
        ...
        async def sustained_operation_benchmark():
            ...
            ), f"Performance degraded by factor of {degradation_factor:.2f}"
        ...
        assert (
            result < 100.0
        ), f"Sustained operation latency {result:.2f}ms exceeds 100ms requirement"
