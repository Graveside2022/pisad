...
from src.backend.core.dependencies import ServiceManager, get_service_manager
...
async def test_service_initialization_order(service_manager):
    ...
    with patch('src.backend.core.dependencies.SDRService') as MockSDR, \
        ...
        expected_order = ['sdr', 'mavlink', 'state_machine', 'signal_processor', 'homing_controller']
...
async def test_service_communication():
    ...
    with patch('src.backend.core.dependencies.SDRService') as MockSDR, \
        ...
        MockHomingController.assert_called_once_with(
            mavlink_service=mock_mavlink,
            signal_processor=mock_signal,
            state_machine=mock_state
        )
