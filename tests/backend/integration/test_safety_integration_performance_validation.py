...
class TestSafetyIntegrationPerformanceValidation:
    ...
    async def test_emergency_stop_timing_under_coordination_load(
        ...
        assert max_response_time < 500.0, f"Max emergency stop time {max_response_time:.2f}ms > 500ms limit"
        assert avg_response_time < 250.0, f"Average emergency stop time {avg_response_time:.2f}ms should be well under limit"
    ...
    async def test_safety_decision_latency_with_coordination(
        ...
        for level, decision_type in decision_types:
            ...
            assert avg_time < 50.0, f"Safety decision {decision_type} avg time {avg_time:.2f}ms > 50ms"
    ...
    async def test_coordination_overhead_impact_on_safety_timing(
        ...
        assert coordination_avg < 100.0, f"Coordination timing {coordination_avg:.2f}ms should be well under 500ms"
    ...
    async def test_sustained_coordination_performance_impact(
        ...
        if len(safety_response_times) >= 2:
            ...
            assert degradation_factor < 1.5, f"Safety performance degraded {degradation_factor:.2f}x over time"
    ...
    async def test_fallback_timing_performance_validation(
        ...
        for i in range(test_iterations):
            ...
            assert fallback_time < 10.0, f"Fallback detection {i} took {fallback_time:.2f}s, exceeds 10s limit"
            assert safe_source in ["drone", "auto", "ground"], f"Fallback recommendation '{safe_source}' invalid"
        ...
        assert max_fallback_time < 10.0, f"Maximum fallback time {max_fallback_time:.2f}s exceeds limit"
    ...
    async def test_concurrent_operations_performance_impact(
        ...
        assert avg_coordination_time < 50, f"Average coordination time {avg_coordination_time:.2f}ms too high"
        assert avg_safety_time < 10, f"Average safety monitoring time {avg_safety_time:.2f}ms too high"
        ...
        assert emergency_time < 500, f"Post-load emergency stop {emergency_time:.2f}ms exceeds 500ms"
        assert emergency_decision.decision_type == SafetyDecisionType.FORCE_EMERGENCY_STOP
    ...
    async def test_deterministic_timing_consistency_validation(
        ...
        assert emergency_coeff_var < 0.5, f"Emergency stop timing variability {emergency_coeff_var:.3f} too high"
        assert decision_coeff_var < 0.5, f"Safety decision timing variability {decision_coeff_var:.3f} too high"
        ...
        assert max(emergency_stop_times) < 500, f"Emergency stop max {max(emergency_stop_times):.2f}ms exceeds limit"
    ...
    async def test_memory_resource_performance_validation(
        ...
        assert memory_increase < 10, f"Memory increased by {memory_increase:.1f}MB, indicates potential memory leak"
        assert avg_cpu_usage < 50, f"CPU usage {avg_cpu_usage:.1f}% too high during sustained operations"
        assert operations_performed > 1000, f"Only performed {operations_performed} operations in {operation_duration}s"
        ...
        assert emergency_time < 500, f"Post-resource-test emergency stop {emergency_time:.2f}ms exceeds 500ms"
        assert emergency_decision.decision_type == SafetyDecisionType.FORCE_EMERGENCY_STOP
