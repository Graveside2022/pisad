...
from src.backend.utils.safety import SafetyEventType, SafetyInterlockSystem, SafetyTrigger
...
class TestSafetyFallbackScenarios:
    ...
    async def test_safety_fallback_on_communication_loss(
        ...
        dual_coordinator.get_health_status = AsyncMock(
            side_effect=Exception("No communication")
        )
    ...
    @pytest.mark.asyncio
    async def test_automatic_drone_only_fallback(
    ...
    @pytest.mark.asyncio
    async def test_coordination_timeout_fallback(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_signal_source_fallback_on_conflict(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_both_sources_fail_scenario(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_coordination_disabled_fallback(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_gradual_communication_degradation(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_coordination_recovery_after_failure(
        self, safety_system, dual_coordinator
    ):
        ...
        dual_coordinator.get_health_status = AsyncMock(
            side_effect=Exception("Initial failure")
        )
