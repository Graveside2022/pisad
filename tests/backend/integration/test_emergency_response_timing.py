...
class TestEmergencyResponseTiming:
    ...
    @pytest.mark.asyncio
    async def test_emergency_stop_timing_with_coordination(
        ...
        assert response_time_ms < 500.0, f"Emergency stop took {response_time_ms:.1f}ms, exceeds 500ms limit"
        
        # Should also be reasonably fast (target <200ms)
        assert response_time_ms < 200.0, f"Emergency stop took {response_time_ms:.1f}ms, should be under 200ms"
    ...
    @pytest.mark.asyncio
    async def test_emergency_stop_timing_under_load(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_safety_interlock_timing_with_coordination(
        self, safety_system, dual_coordinator
    ):
        ...
        assert check_time_ms < 100.0, f"Safety checks took {check_time_ms:.1f}ms, should be under 100ms"
    ...
    @pytest.mark.asyncio
    async def test_coordination_health_check_timing(
        self, safety_system, dual_coordinator
    ):
    ...
    @pytest.mark.asyncio
    async def test_dual_source_signal_check_timing(
        self, safety_system, dual_coordinator
    ):
    ...
    async def test_emergency_stop_timing_with_slow_network(
        ...
        assert response_time_ms < 500.0, f"Emergency stop with slow network took {response_time_ms:.1f}ms"

    @pytest.mark.asyncio
    async def test_repeated_emergency_stop_timing(
        self, safety_system, dual_coordinator
    ):
        ...
        assert max_time < 3 * avg_time, f"Inconsistent timing: max={max_time:.1f}ms, avg={avg_time:.1f}ms"
    ...
    async def test_safety_timing_regression_test(self, safety_system, dual_coordinator):
        ...
        assert overhead_ratio < 5.0, f"Coordination overhead too high: {overhead_ratio:.1f}x baseline"
