...
class TestSDRPPLoadTesting:
    ...
    async def test_concurrent_client_connection_infrastructure(self, load_test_service):
        ...
        for client_result in connected_clients:
            ...
            ), f"Client connection failed: {client_result}"
    ...
    async def _create_test_client(self, port, client_id):
        ...
        message = {"type": "heartbeat", "client_id": client_id, "timestamp": time.time()}
    ...
    async def test_performance_monitoring_under_load(self, load_test_service):
        ...
        if latency_measurements:
            ...
            assert (
                max_latency < 0.15
            ), f"Max latency {max_latency:.3f}s significantly exceeds threshold"
    ...
    async def test_connection_stability_and_memory_usage(self, load_test_service):
        ...
        for cycle in range(num_cycles):
            ...
            for i in range(clients_per_cycle):
                task = asyncio.create_task(
                    self._cycling_client_load(
                        service.port, f"cycle_{cycle}_client_{i}", cycle_duration
                    )
                )
                client_tasks.append(task)
            ...
            for result in results:
                assert not isinstance(
                    result, Exception
                ), f"Connection stability test failed: {result}"
            ...
            assert (
                memory_growth < 50
            ), f"Memory growth {memory_growth:.1f}MB exceeds threshold after cycle {cycle}"
        ...
        assert (
            total_growth < 100
        ), f"Total memory growth {total_growth:.1f}MB exceeds acceptable threshold"
    ...
    async def test_automatic_reconnection_resilience_under_load(self, load_test_service):
        ...
        for result in results:
            assert not isinstance(
                result, Exception
            ), f"Reconnection resilience test failed: {result}"
    ...
    async def _reconnecting_client_load(self, port, client_id, duration):
        ...
        while time.time() - start_time < duration:
            try:
                ...
                while time.time() - connection_start < 1.0 and time.time() - start_time < duration:
                    message = {
                        "type": "heartbeat",
                        "client_id": client_id,
                        "timestamp": time.time(),
