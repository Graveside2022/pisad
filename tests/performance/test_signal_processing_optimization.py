...
class TestVectorizedSignalProcessing:
    ...
    def test_samples_batch(self):
        ...
        return np.random.random((batch_size, samples_per_batch)) + 1j * np.random.random((batch_size, samples_per_batch))

    def test_vectorized_rssi_computation_performance_improvement(self, signal_processor, test_samples_batch):
        """
        ...
        for i, result in enumerate(batch_results):
            ...
            assert abs(result.rssi - single_result.rssi) < 0.1, "Vectorized results should match single processing"
    ...
    def test_optimized_fft_window_application(self, signal_processor, test_samples_single):
        ...
        windowed_samples = signal_processor.apply_window_optimized(test_samples_single, inplace=True)
        ...
        assert window_time < 0.0001, f"Window application should be <0.1ms, got {window_time*1000:.3f}ms"
    ...
    def test_vectorized_power_computation(self, signal_processor):
        ...
        for i in range(num_samples):
            ...
            samples = amplitude * (np.random.random(sample_length) + 1j * np.random.random(sample_length))
        ...
        assert computation_time < 0.001, f"Vectorized power computation should be <1ms, got {computation_time*1000:.3f}ms"
    ...
    def test_optimized_noise_floor_estimation(self, signal_processor):
        ...
        assert estimation_time < 0.001, f"Noise floor estimation should be <1ms, got {estimation_time*1000:.3f}ms"
    ...
    def test_batch_snr_computation_vectorized(self, signal_processor):
        ...
        # Performance should be much faster than individual computation
        assert computation_time < 0.0005, f"Vectorized SNR computation should be <0.5ms, got {computation_time*1000:.3f}ms"
