...
class TestCallbackCircuitBreaker:
    ...
    def test_circuit_half_open_after_timeout(self):
        ...
        config = CircuitBreakerConfig(failure_threshold=1, timeout=timedelta(milliseconds=100))
        breaker = CallbackCircuitBreaker(config=config)
    ...
    def test_circuit_closes_after_success_threshold(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=1, success_threshold=2, timeout=timedelta(milliseconds=100)
        )
        breaker = CallbackCircuitBreaker(config=config)
