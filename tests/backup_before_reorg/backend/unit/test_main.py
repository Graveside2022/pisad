...
class TestMain:
    ...
    def test_main_starts_server(self, mock_config, mock_logging, mock_uvicorn):
        ...
        mock_config.return_value = MagicMock(
            app=MagicMock(APP_NAME="PISAD", APP_HOST="0.0.0.0", APP_PORT=8000),
            development=MagicMock(DEV_HOT_RELOAD=True),
        ...
        mock_uvicorn.assert_called_once_with(
            "src.backend.core.app:app", host="0.0.0.0", port=8000, reload=True, log_level="info"
        )
    ...
    def test_main_production_config(self, mock_config, mock_logging, mock_uvicorn):
        ...
        mock_config.return_value = MagicMock(
            app=MagicMock(APP_NAME="PISAD", APP_HOST="127.0.0.1", APP_PORT=8080),
            development=MagicMock(DEV_HOT_RELOAD=False),
            logging=MagicMock(LOG_LEVEL="WARNING"),
        )
    ...
    def test_main_passes_log_level_lowercase(self, mock_config, mock_logging, mock_uvicorn):
        ...
        mock_config.return_value = MagicMock(
            app=MagicMock(APP_NAME="PISAD", APP_HOST="0.0.0.0", APP_PORT=8000),
            development=MagicMock(DEV_HOT_RELOAD=False),
            logging=MagicMock(LOG_LEVEL="DEBUG"),
        )
