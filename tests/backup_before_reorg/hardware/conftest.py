...
def pytest_configure(config: Any) -> None:
    ...
    config.addinivalue_line("markers", "sdr: mark test as requiring SDR hardware")
    config.addinivalue_line("markers", "mavlink: mark test as requiring MAVLink/flight controller")
...
def pytest_collection_modifyitems(config: Any, items: list) -> None:
    ...
    skip_hardware = pytest.mark.skip(
        reason="Hardware tests disabled (set HARDWARE_TESTS=true to enable)"
    )
    ...
    for item in items:
        ...
        if (
            ...
            and "mock_hardware" not in item.keywords
            and not hardware_available
        ):
...
@pytest.fixture
def real_hackrf(skip_without_hardware: None) -> Generator[HackRFInterface | None, None, None]:
...
@pytest.fixture
def real_mavlink(skip_without_hardware: None) -> Generator[MAVLinkInterface | None, None, None]:
