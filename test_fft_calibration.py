...
def test_fft_rssi_calibration():
    ...
    if error > 3.0:
        ...
        print(f"   Need to adjust calibration_offset from {signal_processor.calibration_offset} to approximately {signal_processor.calibration_offset + (expected_rssi - rssi_fft):.1f}")
        return False
    else:
