...
def test_brief_spike_after_signal_loss():
    ...
    for i, snr in enumerate(snr_pattern):
        ...
        if i in [19, 22, 39, 42, 59, 62, 64, 67]:
            print(
                f"Sample {i:3d}: SNR={snr:5.1f}dB, State={state:15s}, "
    ...
    assert (
        states[23] == "SIGNAL_DETECTED"
    ), f"Should detect after debounced trigger, got {states[23]}"
    print("✓ Sample 23: Correctly detected after 3+ samples above trigger")

    # Should maintain in hysteresis zone (sample 45)
    assert (
        states[45] == "SIGNAL_DETECTED"
    ), f"Should maintain state between thresholds, got {states[45]}"
    print("✓ Sample 45: Correctly maintained in hysteresis zone")
    ...
    print("✓ Sample 67: Correctly dropped after sustained low signal")
