...
for i, snr in enumerate(test_sequence, start=58):
    ...
    print(f"\n--- Sample {i}: SNR={snr}dB ---")
    print(
        f"Before: is_detecting={signal_processor.is_detecting}, det_count={signal_processor.detection_count}, loss_count={signal_processor.loss_count}"
    )
    ...
    print(
        f"After: is_detecting={signal_processor.is_detecting}, det_count={signal_processor.detection_count}, loss_count={signal_processor.loss_count}"
    )
