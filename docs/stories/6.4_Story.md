...
1. **Your GCS Computer** = Command center with two main apps:
   ...
   - **SDR++/SigDigger** = Radio listening station that shows signal strength
...
### **COMPLEMENTARY DUAL-SYSTEM PROFESSIONAL ARCHITECTURE**
```
[COMPLETE PROFESSIONAL SAR PLATFORM - BOTH SYSTEMS WORKING TOGETHER]

┌─────────────────────────────────────────────────────────────────────────────────┐
│                            GROUND CONTROL STATION                               │
│  ┌─────────────────────────────────────────────────────────────────────────────┐ │
│  │                    MISSION PLANNER + SDR++ INTEGRATION                     │ │
│  │                                                                             │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐      │ │
│  │  │ MAVLink     │  │ SDR++ with  │  │ React Web   │  │ Enhanced    │      │ │
│  │  │ Parameters  │  │ PISAD Plugin│  │ Interface   │  │ RF Telemetry│      │ │
│  │  │ • RF Freq   │  │ • Ground    │  │ • ASV .NET  │  │ • RSSI Graph│      │ │
│  │  │ • Homing    │  │   Monitoring│  │   Status    │  │ • Bearing   │      │ │
│  │  │ • Emergency │  │ • Signal    │  │ • d3-waterfall│ │ • Confidence│      │ │
│  │  │ • Override  │  │   Analysis  │  │ • Multi-Analyzer│ │ • Detection │      │ │
│  │  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘      │ │
│  └─────────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────────┘
                                │ MAVLink + TCP Bridge
                                ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│                      RASPBERRY PI 5 (PISAD + ASV + SDR++ BRIDGE)              │
│                                                                                 │
│  ┌───────────────────────────────────────────────────────────────────────────┐ │
│  │                     DUAL-MODE PROCESSING SYSTEM                           │ │
│  │                                                                           │ │
│  │  ┌─────────────────┐    ┌─────────────────────────────────┐               │ │
│  │  │ SDR++ Plugin    │    │      ASV .NET Framework        │               │ │
│  │  │ TCP Bridge      │◄──►│   • Multi-Analyzer Coordinator │               │ │
│  │  │ • Ground Sync   │    │   • GP Analyzer (406 MHz)      │               │ │
│  │  │ • RSSI Stream   │    │   • VOR Analyzer (108-118 MHz) │               │ │
│  │  │ • Status Share  │    │   • LLZ Analyzer (108-112 MHz) │               │ │
│  │  │ • Health Check  │    │   • Professional Processing    │               │ │
│  │  └─────────────────┘    └─────────────────────────────────┘               │ │
│  │         ▲                            ▲                                    │ │
│  │         │ TCP Bridge                 │ Python Bridge                      │ │
│  │         │ (Port 8081)               │ (pythonnet + clr)                  │ │
│  └─────────┼────────────────────────────┼─────────────────────────────────────┘ │
│            │                            │                                      │
│  ┌─────────▼────────────────────────────▼─────────────────────────────────────┐ │
│  │                    ENHANCED SIGNAL PROCESSING PIPELINE                    │ │
│  │                                                                           │ │
│  │  • **DUAL VALIDATION**: Ground SDR++ + Drone ASV cross-verification       │ │
│  │  • **PROFESSIONAL ACCURACY**: ±2° bearing with ASV algorithms             │ │
│  │  • **GROUND INTELLIGENCE**: SDR++ provides RF environment analysis        │ │
│  │  • **AUTONOMOUS PROCESSING**: ASV .NET handles real-time homing decisions │ │
│  │  • **OPERATOR INTERFACE**: Mission Planner + React Web + SDR++ Plugin     │ │
│  └───────────────────────────────────────────────────────────────────────────┘ │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────────┐ │
...
│  │  • Dual-SDR Coordination View (ground + drone signal comparison)           │ │
│  └─────────────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────────────┘
                                │ USB Interface
                                ▼
...
│ Flight Control  │  MAVLink  │ - PISAD Software│
│ - GPS/IMU      │           │ - React Web UI  │
│ - Safety Systems│           │ - Signal Proc   │
└─────────────────┘           └─────────────────┘
                                       │ USB
                                       ▼
                              ┌─────────────────┐
...
## **COMPLEMENTARY OPERATIONAL WORKFLOW (SDR++ Plugin + ASV .NET)**

`★ Insight ─────────────────────────────────────`
**This is the key advantage**: Operators get BOTH professional signal analysis (SDR++ Plugin) AND autonomous processing power (ASV .NET). The SDR++ plugin provides ground station intelligence and coordination, while ASV .NET delivers professional-grade autonomous homing algorithms.
`─────────────────────────────────────────────────`

### **Phase 1: Professional Signal Intelligence (SDR++ Plugin Ground Station) - 3 minutes**

**Step 1: Launch SDR++ with PISAD Bridge Plugin** 
```bash
# Operator launches SDR++ with PISAD plugin installed
sdrpp --config pisad_bridge_config.json
# SDR++ automatically loads PISAD Bridge Module
```

**What the SDR++ Plugin Provides:**
- **Professional Signal Analysis Interface**: Familiar SDR++ waterfall display
- **Ground Station Intelligence**: Complete RF environment analysis  
- **PISAD Bridge Connection**: Real-time connection to drone PISAD system (TCP Port 8081)
- **Dual-SDR Coordination**: Ground HackRF coordinates with drone HackRF
- **Connection Health Monitoring**: Real-time latency, status, and error reporting

**Step 2: Connect SDR++ Plugin to Drone PISAD System**
```
SDR++ PISAD Bridge Plugin Interface:
┌─────────────────────────────────────────────────────┐
│ PISAD Bridge v1.0                                   │
├─────────────────────────────────────────────────────┤
│ Connection Status: [🟢 CONNECTED] Drone Pi         │
│ Host: 192.168.1.100  Port: 8081  Latency: 15ms     │
│                                                     │
│ Ground Station Coordination:                        │
│ • Current Frequency: 406.125 MHz                   │
│ • Ground RSSI: -65 dBm (Strong)                    │
│ • Drone RSSI: -78 dBm (Real-time from drone)       │
│ • Signal Confidence: 87% (ASV .NET Processing)     │
│                                                     │
│ [Send Frequency to Drone] [Sync Analysis] [Test]   │
└─────────────────────────────────────────────────────┘
```

**What the Operator Accomplishes:**
- **Professional RF Analysis**: Uses familiar SDR++ interface for signal discovery
- **Real-Time Drone Coordination**: Live connection shows what drone HackRF is detecting
- **Cross-Verification**: Compare ground station signals with drone measurements  
- **Frequency Coordination**: Send optimal frequencies directly to drone from SDR++
- **Signal Validation**: Confirm beacon detection before enabling autonomous homing

### **Phase 2: ASV .NET Integration Configuration (React Professional Interface) - 2 minutes**

**Step 3: Access Integrated Professional Interface** 
```bash
# Operator opens React web interface with integrated systems view
firefox http://192.168.1.100:3000/professional-sar
```

**Step 4: Professional SAR Configuration Interface**
```
PROFESSIONAL SAR CONTROL DASHBOARD:
┌─────────────────────────────────────────────────────────────────────────┐
│ 🎯 PROFESSIONAL SAR PLATFORM - INTEGRATED CONTROL                     │
├─────────────────────────────────────────────────────────────────────────┤
│ ┌─── SDR++ PLUGIN STATUS ────┐  ┌─── ASV .NET PROCESSING ─────────────┐ │
│ │ 🟢 CONNECTED to Ground     │  │ 🟢 Multi-Analyzer: READY            │ │
│ │ Ground RSSI: -65 dBm       │  │ GP Analyzer: 406 MHz [ACTIVE]       │ │
│ │ Latency: 15ms              │  │ VOR Analyzer: 108-118 MHz [STANDBY] │ │
│ │ Last Sync: 2s ago          │  │ LLZ Analyzer: 108-112 MHz [STANDBY] │ │
│ └────────────────────────────┘  │ Processing Mode: PROFESSIONAL        │ │
│                                 │ Bearing Accuracy: ±2° CERTIFIED     │ │
│ ┌─── CONFIGURATION ──────────┐  └─────────────────────────────────────┘ │
│ │ Target Freq: [406.125] MHz │                                         │ │
│ │ (From SDR++ Discovery)     │  ┌─── LIVE STATUS COMPARISON ────────┐ │
│ │ Search BW: [±2] MHz        │  │ Source      RSSI    Confidence     │ │
│ │ Profile: [GP Emergency]    │  │ Ground:     -65dBm  N/A (Analysis) │ │
│ │ Homing: [☐] SAFE MODE      │  │ Drone ASV:  -78dBm  87% (Professional)│ │
│ └────────────────────────────┘  │ Delta:      +13dBm  VALIDATED      │ │
│                                 └─────────────────────────────────────┘ │
│ ┌─── d3-waterfall PROFESSIONAL DISPLAY ─────────────────────────────────┐ │
│ │                                                                       │ │
│ │ 405.5    406.0    406.125   406.5  MHz                               │ │
│ │   │       │        ▓█▓      │                                        │ │
│ │   ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓████▓▓▓▓▓▓▓▓▓  ← DUAL VALIDATION                  │ │
│ │                  ████  ← Ground + Drone Correlation                   │ │  
│ │ SIGIDWIKI Classification: Emergency Beacon 406.125 MHz [VERIFIED]    │ │
│ │ Ground Analysis: Signal Strength Excellent, Clear Frequency          │ │
│ │ ASV Processing: Professional Mode Active, ±2° Accuracy Confirmed     │ │
│ └───────────────────────────────────────────────────────────────────────┘ │
│                                                                           │
│ [🔄 Sync with SDR++] [🎯 Enable ASV Homing] [🚨 Emergency Stop]         │
└───────────────────────────────────────────────────────────────────────────┘
```

**What This Professional Interface Provides:**
1. **Integrated System Status**: See both SDR++ plugin and ASV .NET status simultaneously
2. **Dual-System Validation**: Ground station analysis confirms drone measurements  
3. **Professional Signal Processing**: ASV .NET provides ±2° bearing accuracy
4. **d3-waterfall Visualization**: Professional-grade signal analysis display
5. **SIGIDWIKI Integration**: Automatic signal classification and verification
6. **Real-Time Correlation**: Live comparison between ground and drone measurements

`★ Insight ─────────────────────────────────────`
**This is the "Ferrari" advantage**: The operator gets professional RF analysis tools (SDR++ familiar interface) combined with autonomous processing power (ASV .NET professional algorithms). No other SAR system provides this dual-capability integration.
`─────────────────────────────────────────────────`

### **Phase 3: Integrated Mission Execution (All Systems Coordinated) - Mission Duration**

**Step 5: Mission Planner with Integrated Systems Display**
```
MISSION PLANNER - ENHANCED RF INTEGRATION VIEW:
┌───────────────────────────────────────────────────────────────────────────┐
│ FLIGHT DATA - Professional SAR Platform Integration                      │
├───────────────────────────────────────────────────────────────────────────┤
│  ┌──────────┐    ATTITUDE    ┌─────────── CONNECT ──┐                   │
...
│  └──────────┘                └───────────────────────┘                   │
│                                                                           │
│ ┌─── INTEGRATED RF STATUS (SDR++ + ASV .NET) ──────────────────────────┐ │
│ │ Ground SDR++:   -65 dBm (Strong) | Drone ASV: -78 dBm (87% Conf)    │ │
│ │ Frequency Sync: ✅ MATCHED       | Processing: Professional ±2°      │ │
│ │ Plugin Status:  🟢 CONNECTED     | Multi-Analyzer: GP ACTIVE        │ │
│ │ Bearing (ASV):  142° (SE)        | Homing Status: SAFE MODE         │ │
│ └───────────────────────────────────────────────────────────────────────┘ │
│                                                                           │
│ ┌─── DUAL-SYSTEM VALIDATION ─────────────────────────────────────────┐   │
│ │ Cross-Check: Ground vs Drone Signal Correlation: ✅ VERIFIED       │   │
│ │ ASV Confidence: 87% (Professional Processing)                      │   │  
│ │ SDR++ Analysis: Signal Clear, Optimal Frequency Selected           │   │
│ │ Ready for Homing: All Systems Validated ✅                         │   │
│ └─────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────────────────────────────────────────┘
```

**Step 6: Enable Professional ASV Homing with SDR++ Monitoring**
```bash
# Mission Planner Parameter: Enable ASV .NET Professional Homing
PISAD_HOMING_ENABLE = 1
PISAD_MODE = "ASV_PROFESSIONAL" 
# SDR++ Plugin automatically enters monitoring mode
# ASV .NET takes control with professional ±2° algorithms
```

**Step 7: Coordinated Autonomous Operation**
- **ASV .NET Framework**: Handles autonomous homing with professional algorithms
- **SDR++ Plugin**: Continues ground station monitoring and cross-validation
- **Mission Planner**: Shows integrated telemetry from both systems
- **React Interface**: Displays real-time status from SDR++ plugin + ASV processing
- **Operator Benefits**: Professional tools + autonomous operation + integrated monitoring

## **COMPLEMENTARY SYSTEM ADVANTAGES (SDR++ Plugin + ASV .NET)**

### **SDR++ Plugin Professional Benefits:**
✅ **Familiar Interface**: Operators use tools they already know (SDR++/SigDigger)
✅ **Ground Station Intelligence**: Complete RF environment analysis with professional waterfall displays
✅ **Real-Time Drone Coordination**: Live TCP bridge shows what drone is detecting  
✅ **Cross-Validation**: Compare ground measurements with drone ASV processing
✅ **Professional Signal Analysis**: ImPlot graphs, RSSI meters, connection health monitoring
✅ **Frequency Coordination**: Send optimal frequencies directly from SDR++ to drone
✅ **Connection Resilience**: Exponential backoff, heartbeat monitoring, graceful degradation
...
✅ **Best of Both Worlds**: Professional analysis tools + autonomous processing power
✅ **Dual-System Validation**: Ground station confirms drone measurements in real-time  
✅ **Unified Interface**: Mission Planner shows integrated telemetry from both systems
✅ **Professional Workflow**: Familiar SDR++ interface + React web dashboard + Mission Planner
✅ **Comprehensive Monitoring**: SDR++ plugin monitors while ASV .NET processes autonomously
✅ **Emergency Coordination**: Both systems provide emergency override and safety integration
✅ **Operational Flexibility**: Use SDR++ for analysis, ASV .NET for execution, integrated monitoring

### **UPDATED SYSTEM DIAGRAMS FOR DUAL-HACKRF WORKFLOW**

**GROUND STATION (Your Computer) - Signal Discovery Phase:**
```
┌─────────────────────────────────────┐
│  YOUR GCS COMPUTER                  │
│  IP: 192.168.1.200                 │
...
│                    └─────────────┘   │
└─────────────────────────────────────┘
           ↓ (Network Connection to Drone)
         📡 Configure drone via web interface
```

**DRONE SYSTEM - Autonomous Homing Phase:**
```
┌─────────────────────────────────────┐
...
🚨 KEY: HackRF #1 = Discovery Only
        HackRF #2 = Homing Only
...
### **SIGDIGGER/SDR++ INTERFACE LAYOUT (Ground Station Signal Discovery)**

```
┌────────────────────────────────────────────────────────────┐
│ SigDigger/SDR++ - Ground Station Signal Discovery         │
├────────────────────────────────────────────────────────────┤
│                                                            │
│ Device: [HackRF #1 Ground]     Gain: [30] dB              │
│ Center Freq: [406.000000] MHz  Sample Rate: [2.048] MHz   │
│ Mode: [🔍 DISCOVERY MODE]      Status: [SCANNING]          │
│                                                            │
│ ┌─ SPECTRUM WATERFALL DISPLAY ─────────────────────────┐   │
│ │                                                     │   │
│ │  405.5    406.0    406.5  MHz                       │   │
│ │    │       │       │                               │   │
│ │    ▓▓▓▓▓▓▓▓█▓▓▓▓▓▓▓  ← TARGET BEACON (Bright)       │   │
│ │    ▓▓▓▓▓▓▓▓█▓▓▓▓▓▓▓                               │   │
│ │    ▓▓▓▓▓▓▓▓█▓▓▓▓▓▓▓  Time flows ↓                │   │
│ │    ▓▓▓░░░░░█░░░░▓▓▓                               │   │
...
│ └─────────────────────────────────────────────────────┘   │
│                                                            │
│ ✅ Target Found: 406.125 MHz @ -65 dBm [STRONG SIGNAL]    │
│ 📝 Next Step: Configure drone HackRF #2 via web interface │
│                                                            │
│ ⚠️  Interference detected at 405.8 MHz - avoid this freq  │
│ 💡 Recommendation: Use 406.125 MHz for best performance   │
└────────────────────────────────────────────────────────────┘
```

### **WHAT EACH INTERFACE MEANS IN DUAL-HACKRF WORKFLOW**

**Ground Station SigDigger/SDR++ (HackRF #1 - Discovery Phase):**
- **🔍 DISCOVERY MODE**: Ground station signal analysis only
- **Device [HackRF #1 Ground]**: Shows you're using ground station radio
- **Waterfall Display**: Visual spectrum analysis of RF environment
- **Target Found**: Shows discovered beacon frequency and strength  
- **Interference Detection**: Identifies competing signals to avoid
- **Recommendation**: Suggests optimal frequency for drone configuration
- **📝 Next Step**: Tells you to configure drone - no direct connection to PISAD

**Mission Planner Controls (Drone HackRF #2 - Configuration & Monitoring):**
- **[CONNECT] Button**: Links Mission Planner to your drone flight controller
- **PISAD_RF_FREQ**: Configure drone HackRF #2 frequency (from discovery phase)
- **PISAD_HOMING_ENABLE**: The "GO" button (0=Safe, 1=Active homing mode)
- **[WRITE PARAMS]**: Saves frequency settings to drone HackRF #2

**Mission Planner Status Displays (Live from Drone HackRF #2):**
- **PISAD_RSSI**: Signal strength from **drone HackRF #2** (-30=Strong, -999=None)
- **PISAD_SIG_CONF**: Drone's confidence level (70%+ = Good for homing)  
- **PISAD_BEARING**: Direction to fly from **drone HackRF #2** (0°=North)
- **PISAD_HOMING**: Current homing status (DISABLED/ACTIVE/HOMING)

**🚨 CRITICAL UNDERSTANDING:**
- **Ground HackRF #1**: Only for pre-mission signal discovery
- **Drone HackRF #2**: Only for mission execution and homing
- **No real-time coordination**: Each HackRF works independently
- **Mission Planner shows drone data only**: All telemetry from HackRF #2
