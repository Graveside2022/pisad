...
### **ASV .NET INTEGRATION OPERATIONAL WORKFLOW (Professional SAR Platform)**

**The system integrates ASV Drones SDR Framework (.NET) with PISAD for professional-grade signal processing:**

## **ASV .NET Architecture Overview**

### **Professional Signal Processing Stack**
```
[ASV PROFESSIONAL SAR PLATFORM]
┌─────────────────────────────────────────────────────────────────┐
│                    Mission Planner Integration                   │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  │
│  │  MAVLink Params │  │  Enhanced HUD   │  │  RF Telemetry   │  │
│  │  • PISAD_RF_*   │  │  • RSSI Graph   │  │  • Confidence   │  │
│  │  • Real-time    │  │  • Bearing ±2°  │  │  • Signal Class │  │
│  │    Control      │  │  • Signal Data  │  │  • Detection    │  │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
                                │ MAVLink Protocol
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                   Raspberry Pi 5 (PISAD + ASV)                 │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────────┐ │
│  │              ASV .NET Integration Layer                     │ │
│  │                                                             │ │
│  │  ┌─────────────────┐    ┌─────────────────────────────────┐ │ │
│  │  │  Python Bridge  │    │    ASV .NET Framework          │ │ │
│  │  │  • pythonnet    │◄──►│  • Multi-Analyzer Coordinator  │ │ │
│  │  │  • clr_loader   │    │  • GP Analyzer (406 MHz)       │ │ │
│  │  │  • AsyncIO      │    │  • VOR Analyzer (108-118 MHz)  │ │ │
│  │  │  • Task Bridge  │    │  • LLZ Analyzer (108-112 MHz)  │ │ │
│  │  └─────────────────┘    └─────────────────────────────────┘ │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                │                                 │
│  ┌─────────────────────────────▼─────────────────────────────────┐ │
...
│  │                                                             │ │
│  │  • Professional ±2° Bearing Calculations                   │ │
│  │  • Confidence-Based Homing (0.1-1.0 scale)               │ │
│  │  • Multi-Frequency Coordination (GP/VOR/LLZ)              │ │
│  │  • Interference Rejection & Classification                 │ │
│  │  • Real-time Performance: <50ms switching latency         │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                │                                 │
│  ┌─────────────────────────────▼─────────────────────────────────┐ │
│  │              d3-waterfall Professional Visualization       │ │
│  │                                                             │ │
│  │  • RTL-SDR Style Professional Interface                    │ │
│  │  • SIGIDWIKI Signal Database Integration                   │ │
│  │  • Auto Signal Detection & Classification                  │ │
│  │  • Interactive Annotations & Tooltips                      │ │
│  │  • Real-time Spectrum Analysis                            │ │
│  │  • Zoom/Pan Performance Optimizations                      │ │
│  └─────────────────────────────────────────────────────────────┘ │
│                                │                                 │
└────────────────────────────────┼─────────────────────────────────┘
                                │ USB Interface
...
### **Phase 1: Signal Discovery (Ground Station) - 5 minutes**

**Step 1: RF Environment Reconnaissance** 
```bash
# Operator launches SigDigger or SDR++ with ground HackRF #1
sigdigger --device hackrf  # or ./sdrpp
```

**What the Operator Does:**
- Connect **HackRF #1** to GCS laptop via USB
- Launch **SigDigger** or **SDR++** signal analysis software
- Scan frequency range where beacon is expected (e.g., 406 MHz area)
- Use waterfall display to **visually identify** target beacon signal
- Look for **bright line** on waterfall = beacon transmitting

**What the Operator Discovers:**
- **Beacon Frequency**: e.g., 406.125 MHz (exact frequency)
- **Signal Strength**: e.g., -65 dBm at ground station location  
- **Signal Quality**: Continuous vs intermittent signal
- **Interference**: Other signals that might cause problems
- **Best Frequency**: Cleanest frequency with strongest signal

### **Phase 2: Drone Configuration (React Web Interface) - 2 minutes**

**Step 2: Connect to Drone PISAD System**
```
Network Connection to Drone:
GCS Laptop ──── WiFi/Radio Link ──── Drone Pi5
            (You handle this link)        │
                                    React Web Server
                                    (Port 3000/8080)
```

**Step 3: Configure RF Homing Parameters via Web Browser**
```
RF Payload Configuration Web Interface:
┌─────────────────────────────────────┐
│ 🎯 RF Homing Configuration         │
├─────────────────────────────────────┤
│ Beacon Frequency: [406.125] MHz    │ ← Enter frequency from Step 1
│ Search Bandwidth: [±2] MHz         │ ← How wide to search  
│ RSSI Threshold:   [-75] dBm        │ ← Minimum signal to detect
│ Homing Enable:    [☐] DISABLED     │ ← Keep OFF until ready
├─────────────────────────────────────┤
│ Current Status from Drone HackRF #2:│
│ Current RSSI: -78.2 dBm            │ ← Live signal strength
│ Signal Detected: ✅ YES             │ ← Drone can hear beacon
│ Signal Quality: 85% GOOD           │ ← Confidence level
│                                     │
│ [Apply Settings] [Test Detection]   │
└─────────────────────────────────────┘
```

**Configuration Steps:**
1. **Set Target Frequency**: Enter exact frequency found in Step 1
2. **Verify Detection**: Check that drone HackRF #2 can hear the beacon  
3. **Confirm Signal Quality**: Make sure confidence is >70%
4. **Apply Settings**: Save configuration to drone
5. **Do NOT enable homing yet** - keep it disabled for safety

### **Phase 3: Mission Execution (Autonomous Drone) - Mission Duration**

**Step 4: Launch Mission via Mission Planner**
- **Take off** using normal Mission Planner flight controls
- **Fly to search area** using waypoint navigation or manual control
- **Monitor RF detection** in Mission Planner telemetry display
- **Verify signal reception** before enabling homing

**Step 5: Enable RF Homing When Ready**
```
Mission Planner RF Status Display:
┌─────────────── RF STATUS ───────────────┐
│ PISAD_RSSI:     -65 dBm    (STRONG)     │ ← Good signal
│ PISAD_SIG_CONF:  87%       (HIGH)       │ ← High confidence  
│ PISAD_BEARING:   142°      (SE)         │ ← Direction to fly
│ PISAD_HOMING:    DISABLED  (SAFE)       │ ← Ready to enable
└─────────────────────────────────────────┘

Enable Homing: Set PISAD_HOMING_ENABLE = 1
```

**Step 6: Monitor Autonomous Operation**
- **Drone flies automatically** toward strongest signal using HackRF #2
- **Ground station HackRF #1** not used during homing (optional monitoring)
- **Operator monitors progress** via Mission Planner telemetry
- **React web interface** shows real-time homing status and signal strength

## **Key Advantages of This Dual-HackRF Approach**

### **Ground HackRF #1 Benefits:**
✅ **Signal Intelligence**: Complete RF environment analysis before mission  
✅ **Interference Detection**: Identify competing signals or jammers  
✅ **Optimal Frequency**: Find cleanest, strongest beacon frequency  
✅ **Mission Planning**: Better situational awareness for operator  
✅ **Tool Flexibility**: Can use SigDigger OR SDR++ for analysis  

### **Drone HackRF #2 Benefits:**  
✅ **Autonomous Operation**: Drone handles all homing logic independently  
✅ **Real-Time Processing**: Live signal processing during flight  
✅ **Safety Integration**: Connected to flight controller safety systems  
✅ **Performance Optimized**: Dedicated to homing task, no distractions  
✅ **Reliable Operation**: No dependency on ground station during homing  

### **Operational Workflow Benefits:**
✅ **Simple Setup**: Each HackRF has dedicated purpose  
✅ **Reliable Mission**: Drone operates independently after configuration  
✅ **Expert Analysis**: Ground station provides professional RF intelligence  
✅ **Flexible Tools**: Use best tool for each job (discovery vs execution)  
✅ **Clear Procedures**: Operator knows exactly what to do at each step
