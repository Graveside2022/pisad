...
### 🎯 Quantitative Results Achieved (Updated - Sprint 8 Day 1-2)
...
### 🆕 Sprint 8 Day 1-2 - CORRECTED BY SENTINEL (2025-08-16)
...
- **Story 4.5:** API Security - Ready to start
...
**Measurement:** Current cyclomatic complexity: 142 → Target: <30 per class
...
## 🎯 EXPECTED OUTCOMES - Post-Optimization System State
...
#### Task 9.4: Parallel Execution Setup [4 points] ✅ COMPLETED
...
- [ ] **Task 8.1c:** Complete Mock Removal (IN PROGRESS)
  ```python
  # Rex's fix_tests.py systematic approach:
  - Remove all mock imports
  - Replace Mock() with None placeholders
  - Add hardware skip decorators
  - Delete non-PRD test files
  # Status: 1 of 74 files processed before cutoff
  ```

- [ ] **Task 8.1d:** Fix 26 Import Errors
  ```bash
  # Files with import errors needing manual fixes:
...
- [ ] **Task 8.1e:** Hardware Integration Blocker
  ```yaml
  # TASKS.md hardware requirements:
  - ArduPilot SITL installation (blocked by permissions)
  - HackRF One or RTL-SDR hardware
  - Cube Orange+ flight controller
  # User must install SITL manually outside project
  ```

- [ ] **Task 8.1f:** Consolidate Test Structure
  - Reduce from 77 test files to ~30 (1 per service)
  - Add pytest markers: unit, integration, hardware
  - Remove test duplication

#### Day 3-4: Delete Non-PRD Features & Tests (13 points)
**Goal:** Remove scope creep features entirely, not just tests

- [ ] **Task 8.4:** DELETE non-PRD feature files
  ```bash
  # DELETE THESE FILES - NOT IN PRD:
  rm src/backend/services/analytics.py
  ...
  rm -rf tests/*/test_analytics*
  rm -rf tests/*/test_recommendations*
  rm -rf tests/*/test_report*
  ```

- [ ] **Task 8.5:** Create PRD-only mock fixtures
  ```python
  # Only 2 mocks needed for PRD:
  @pytest.fixture
  def mock_hackrf():
      """FR1: Beacon detection mock"""
      return Mock(spec=['detect_beacon', 'get_rssi'])

  @pytest.fixture
  def mock_mavlink():
      """FR9: MAVLink telemetry mock"""
      return Mock(spec=['send_rssi', 'recv_command'])
  ```

- [ ] **Task 8.6:** Remove frontend test infrastructure
  ```bash
  # Frontend is UI goals only - not critical
  rm -rf tests/frontend/
  rm package.json # Remove Jest references
  ```

#### Day 5-6: PRD State Machine Tests (8 points)
**Goal:** Test ONLY PRD-required state transitions

- [ ] **Task 8.7:** Test FR3 state transitions
  ```python
  def test_fr3_homing_transition_2_seconds():
      """FR3: Transition to HOMING within 2 seconds"""
      # Must verify <2 second transition time

  def test_fr7_debounced_transitions():
      """FR7: Debounced with 12dB trigger, 6dB drop"""
      # Test thresholds from PRD
  ```

- [ ] **Task 8.8:** Test FR15 safety transitions
  ```python
  def test_fr15_cease_velocity_on_mode_change():
      """FR15: Stop commands when not in GUIDED mode"""
      # Critical safety requirement

  def test_fr17_auto_disable_after_signal_loss():
      """FR17: Disable homing after 10s signal loss"""
      # Timeout safety feature
  ```

#### Day 7: MAVLink Protocol Tests (5 points)
**Goal:** Test FR9 and NFR1 MAVLink requirements

- [ ] **Task 8.9:** Test MAVLink telemetry streaming
  ```python
  def test_fr9_mavlink_rssi_telemetry():
      """FR9: Stream RSSI via MAVLink NAMED_VALUE_FLOAT"""
      # Verify correct message format

  def test_nfr1_mavlink_packet_loss():
      """NFR1: <1% packet loss at 115200 baud"""
      # Measure actual packet loss
  ```

- [ ] **Task 8.10:** Test MAVLink command reception
  ```python
  def test_fr11_gcs_override():
      """FR11: GCS mode changes override payload"""
      # Safety critical - GCS has priority
  ```

#### Day 8: Signal Processing Performance (5 points)
**Goal:** Validate NFR2 latency requirement

- [ ] **Task 8.11:** Test signal processing latency
  ```python
...
#### 📅 Day 4: Execute State Machine Tests (5 points)
...
*Next Review: Sprint 8 Day 3 - Continue with real PRD test implementation*
